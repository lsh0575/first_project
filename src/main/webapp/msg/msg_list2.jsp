...
<div class="body-top container">
	...
	<div id="result">
		...
		<div class="text-center">
			<ul class="pagination" id="page_number">
				<c:if test="${paging.pagesStart!=1}">
					<li>
						<a data-pageStartNum="${(paging.pagesStart-2)*paging.onePageListCount}"
						class="msgbtn btn btn-default" title="이전의 페이지들을 보여줍니다." >이전</a>
					</li>
				</c:if>
				<c:forEach var="dto" begin="${paging.pagesStart}" end="${paging.pagesEnd}" step="1" varStatus="stat">
					<li <c:if test="${stat.index eq paging.currentPages}">class="active"</c:if> >
						<a data-pageStartNum="${(stat.index-1)*paging.onePageListCount}"
						class="msgbtn btn btn-default" title="${stat.index} 페이지로 이동합니다.">${stat.index}</a>
					</li>
				</c:forEach>
				<c:if test="${paging.pageCount!=paging.pagesEnd}">
					<li>
						<a data-pageStartNum="${(paging.pagesEnd)*paging.onePageListCount}"
						class="msgbtn btn btn-default" title="다음의 페이지들을 보여줍니다.">다음</a>
					</li>
				</c:if>
			</ul>
		</div> 
		...
		<div class="form-group text-right">
			<input type="button" value="삭제" id="deletebtn" class="btn btn-danger" title="선택한 글을 삭제합니다."/>
